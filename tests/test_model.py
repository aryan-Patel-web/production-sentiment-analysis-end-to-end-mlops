...
class TestModelLoading(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Loads model, vectorizer, and test data.
        """

        # -------------------------------
        # ✅ Setup DagsHub + MLflow Auth
        # -------------------------------
        dagshub_token = os.getenv("CAPSTONE_TEST")
        if not dagshub_token:
            raise EnvironmentError("CAPSTONE_TEST environment variable is not set")
        ...
        if cls.new_model_version is None:
            raise ValueError("❌ No model found in registry at given stage")
    ...
    def get_latest_model_version(model_name, stage="Staging"):
        """
        Fetch latest model version from MLflow registry.
    ...
    # -------------------------------
    def test_model_performance(self):
        """Ensure model meets minimum accuracy thresholds"""
    ...
    # -------------------------------
    # 🔥 BONUS TEST: Latency Test
    # -------------------------------
    def test_prediction_latency(self):
        """Ensure prediction is fast enough (production readiness)"""

        X_holdout = self.holdout_data.iloc[:, 0:-1]

        start_time = time.time()
        self.new_model.predict(X_holdout)
        end_time = time.time()

        latency = end_time - start_time

        # Expect inference under 2 seconds
        self.assertLess(latency, 2, f"Prediction too slow: {latency:.2f}s")
